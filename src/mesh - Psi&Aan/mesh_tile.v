...
module mesh_tile #(
    ...
    // After adding Noc booting:
    // tile(0,0): must use boot bus — it's the NOC inject source
    // tiles 1-8: use NOC boot flit only
    
    // Boot bus active for ALL tiles (original behavior)
wire boot_bus_writing = boot_mode && ~boot_wen;

// NOC boot only for tiles 1-8 (new behavior)  
...
always @(negedge boot_wen) begin
    if (TILE_ID == 4'h4)  // tile(1,0) = row1,col0 = ID 0100 = 4
        $display("[TILE14 t=%0t] boot_wen received addr=0x%03x data=0x%02x boot_mode=%b",
                 $time, boot_addr, boot_data, boot_mode);
end

    // -----------------------------------------------------------------------
    // WB mux — CPU WB normally, fake pop during NOC boot
    // -----------------------------------------------------------------------
    wire final_wb_stb = boot_pop_active ? 1'b1           : wb_stb;
    wire final_wb_we  = boot_pop_active ? 1'b0           : wb_we;
    wire [31:0] final_wb_adr =
        boot_pop_active ? 32'h80000004 : wb_adr;
    wire [31:0] final_wb_dat =
        boot_pop_active ? 32'h0        : wb_dat_c2r;
