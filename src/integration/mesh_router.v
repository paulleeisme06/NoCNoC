...
//
// TILE_ID encoding (4-bit, in a 6-bit parameter, upper 2 bits zero):
//   [3:2] = row   (0..3)
//   [1:0] = col   (0..3)
//
...
//
// The firmware uses FLIT_DEST_SHIFT=28, placing the 4-bit dest TILE_ID
// at CPU-write-word bits [31:28].  The router extracts bits [31:28] and
// packs them into flit[32:29] (= {dest_row[1:0], dest_col[1:0]}).
//
...
module mesh_router #(
    ...
    // Decode this tile's position from MY_ID  ({row[1:0], col[1:0]})
    ...
    always @(posedge clk) begin
        if (rst) begin
            ...
            if (inject_ack_cycle) begin
                inject_flit <= {1'b1, 2'b00,
                                local_wb_dat_o[31:28],   // dest TILE_ID [3:0]
    ...
    task automatic route_one;
        ...
        begin
            tgt_row = flit[32:31];   // dest row  in bits [32:31]
            tgt_col = flit[30:29];   // dest col  in bits [30:29]
        ...
        if (rst) begin
            ...
            if (inject_ack_cycle)
                $display("[FLIT t=%0t] TILE=%0d INJECT dest_row=%0d dest_col=%0d payload=0x%08x",
                         $time, MY_ID,
                         local_wb_dat_o[31:30], local_wb_dat_o[29:28],
                         local_wb_dat_o[28:0]);
