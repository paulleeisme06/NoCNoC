...
module mesh_tile #(
    ...
    // Host readback (only sampled while boot_mode=1, CPU held in reset)
    input  wire [10:0] host_rd_addr,
...
`endif
    ...
    // -------------------------------------------------------------------------
    // Address / Data MUX
    // Host read takes priority over boot write within boot_mode.
    // -------------------------------------------------------------------------
    wire [10:0] final_a =
        (boot_mode && host_rd_req) ? host_rd_addr :
        boot_mode                  ? boot_addr    :
        sram_wen                   ? sram_waddr   :
                                     sram_raddr;
    ...
    // Read:  CEN enabled, GWEN held HIGH (no write).
    // CPU:   CEN=HIGH for init-pulse cycle, then permanently LOW.
    // -------------------------------------------------------------------------
    wire sram_active =
        (boot_mode && host_rd_req) ? 1'b1     :
        boot_mode                  ? ~boot_wen :
                                     ~cpu_sram_init_pulse;
    wire sram_write  =
        (boot_mode && host_rd_req) ? 1'b0     :
        boot_mode                  ? ~boot_wen :
                                     sram_wen;
    ...
    // -------------------------------------------------------------------------
`ifdef SIMULATION
    wire [10:0] _tb_raddr_w = tb_raddr;
    wire  [7:0] _tb_rdata_w;
    assign tb_rdata = _tb_rdata_w;
`else
    wire [10:0] _tb_raddr_w = 11'b0;
    wire  [7:0] _tb_rdata_w;
`endif
    ...
    gf180mcu_fd_ip_sram__sram2048x8m8wm1 sram_inst (
        ...
        .VDD (),
        .VSS (),
        .tb_raddr(_tb_raddr_w),
        .tb_rdata(_tb_rdata_w)
    );
